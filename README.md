
## Frameworks

#### whs.js
  Super-fast 3D framework for Web Applications 🥇 & Games 🎮. Based on Three.js 
  https://whs.io
  https://github.com/WhitestormJS/whs.js
  Examples: https://whs-dev.surge.sh/examples/
  
#### webglstudio.js
  A full open source 3D graphics editor in the browser, with scene editor, coding pad, graph editor, virtual file system, and many features more.
  https://webglstudio.org/
  https://github.com/jagenjo/webglstudio.js
  
#### ThreeNodes.js
  vvvv(visual programming language, http://www.vvvvjs.com/) "clone"
  http://idflood.github.io/ThreeNodes.js/index_optimized.html#
  
#### Jeeliz Face Filter
  Javascript/WebGL lightweight face tracking library designed for augmented reality webcam filters. Features : multiple faces detection, rotation, mouth opening. 
  https://jeeliz.com 
  https://github.com/jeeliz/jeelizFaceFilter
 
#### CindyJS
  A JavaScript framework for interactive (mathematical) content.
  https://cindyjs.org/
  https://github.com/CindyJS/CindyJS
  https://observablehq.com/@bryangingechen/hello-cindyjs
  https://github.com/CindyJS/CindyJS/tree/master/examples

#### Wagner
  Effects composer for three.js
  https://github.com/spite/Wagner
  Demo：https://www.clicktorelease.com/tmp/wagner/

#### Thingjs (Chinese Only)
  3D visualization PaaS development platform for IoT
  http://www.thingjs.com/
  
#### three.dart
  Dart 3D Engine
  https://github.com/threeDart/three.dart
  Dart: https://dart.dev/ (Dart is a client-optimized language for fast apps on any platform)
  Demo: http://threedart.github.io/three.dart/example/web_gl_geometries/web_gl_geometries.html
  
#### Embedding Projector(Google)
  http://projector.tensorflow.org/


#### w3reality/three-geo
  Geographic visualization library using three.js
  https://github.com/w3reality/three-geo
  https://w3reality.github.io/three-geo/examples/geo-viewer/io/index.html
  
## Plugins 
  #### three.bas
  Buffer Animation System
  https://github.com/zadvorsky/three.bas
  Examples: http://three-bas-examples.surge.sh/
  
  #### google-ar/three.ar.js
  A helper three.js library for building AR web experiences that run in WebARonARKit and WebARonARCore
  https://github.com/google-ar/three.ar.js
  
  #### Panolens.js
  Panolens.js is an event-driven and WebGL based panorama viewer. Lightweight and flexible.
  https://pchen66.github.io/Panolens/#Example

## Physics

#### cannon.js
  Lightweight 3D physics for the web
  https://github.com/schteppe/cannon.js?files=1
  Demos: http://schteppe.github.io/cannon.js/
  3DClothSlideshow: https://tympanus.net/Tutorials/3DClothSlideshow/
  
#### chandlerprall/Physijs
  Physics plugin for Three.js
  https://github.com/chandlerprall/Physijs
  
## Data Visualization (Graphs & Charts)

#### 3d-force-graph[star5]
  3D force-directed graph component using ThreeJS/WebGL
  https://github.com/vasturiano/3d-force-graph
  
#### scene-tree
  Modular scene graph for composing and manipulating objects in a 3D scene.
  https://github.com/hughsk/scene-tree
  Demo: http://hughsk.io/scene-tree/
  
#### visjs/vis-graph3d
  Create interactive, animated 3d graphs. Surfaces, lines, dots and block styling out of the box.
  https://github.com/visjs/vis-graph3d
  Demo: https://github.com/visjs/vis-graph3d/tree/master/examples/graph3d

#### Graph-Visualization[not maintained]
  3D graph visualization
  https://github.com/davidpiegza/Graph-Visualization
  Examples:http://davidpiegza.github.io/Graph-Visualization/index.html

#### vasturiano/three-forcegraph
  Force-directed graph as a ThreeJS 3d object 
  https://github.com/vasturiano/three-forcegraph

#### giojs
  A Declarative 3D Globe Data Visualization Library built with Three.js
  https://giojs.org/
  https://github.com/syt123450/giojs
  Examples: https://giojs.org/html/playground.html
  
#### frewsxcv/graphosaurus [Archived]
  A three-dimensional static graph viewer.
  https://github.com/frewsxcv/graphosaurus
  https://frewsxcv.github.io/graphosaurus/examples/eve-universe/index.html
  
#### Porting d3-3d scatterplot
  This example was ported from this d3-3d repo (https://github.com/Niekes/d3-3d)
  https://observablehq.com/@bumbeishvili/porting-3d-d3-scatterplot-example
  https://observablehq.com/@jamesleesaunders/d3-x3d-components-bubbles
  https://observablehq.com/@jamesleesaunders/d3-x3d-components-bar-charts
  https://observablehq.com/@jamesleesaunders/d3-x3d-components-ribbon-chart
  https://observablehq.com/@jamesleesaunders/d3-x3d-components-surface-plot?collection=@jamesleesaunders/d3-x3d
  
  
#### 3d Area chart - three.js
  https://observablehq.com/@bumbeishvili/3d-area-chart-three-js
  
#### Zdog 3D Force-Directed Graph
  This network of character co-occurence in Les Misérables is positioned by simulated forces using d3-force-3d(https://github.com/vasturiano/d3-force-3d) and displayed using Zdog(https://zzz.dog/).
  https://observablehq.com/@nhogs/zdog-3d-force-directed-graph
  
#### clx. porting 3D Scatter Plot Using three.js
  https://observablehq.com/@bumbeishvili/clx-porting-3d-scatter-plot-using-three-js?collection=@bumbeishvili/3d
  
#### 3d-force-graph nodes clicks and selection
  https://observablehq.com/@artyomtrityak/3d-force-graph-nodes-clicks-and-selection
  http://bl.ocks.org/anonymous/229589e2e98139ef6b957bd5128711aa
  
#### area3d_vis
  3D Graph Visualization for Kibana 5 and 6.0.0-alpha1!
  https://github.com/JuanCarniglia/area3d_vis
  
  
#### Samples scatter plot visualization
  https://observablehq.com/@baygeldin/samples-scatter-plot-visualization
  
#### WebGL 3D Dataviz
  Creating an interactive 3D X/Y graph prototype with WebGL/ThreeJS.
  https://github.com/swieder227/3D-dataviz-prototype
  https://swieder227.github.io/3D-dataviz-prototype/
  
#### 3d-force-graph-vr [VR]
  3D force-directed graph component in VR
  https://github.com/vasturiano/3d-force-graph-vr
  https://vasturiano.github.io/3d-force-graph-vr/example/auto-colored/

## Experiments

#### ScrollSpiral
  WebGL Scroll Spiral  
  https://github.com/codrops/ScrollSpiral
  Demos: https://tympanus.net/Development/ScrollSpiral/index.html
  
#### js1k
  https://js1k.com/ 

#### Procedural Clouds
  https://tympanus.net/Tutorials/ProceduralClouds/
  
#### 3 visualizations based on textual analysis to illustrate an article
  https://twitter.com/ludovicriffault/status/1220644096261459969
  
#### 3D text, PhysicsMenu
  https://tympanus.net/Tutorials/PhysicsMenu/
  
#### Exploding Objects
  https://tympanus.net/Development/ExplodingObjects/index.html
  
#### 3D planets
  A minimal Three.js example of a textured sphere with controls.
  https://observablehq.com/@mourner/3d-planets-with-three-js
  
#### Calabi-Yau Manifold 3D
  Calabi–Yau manifolds are important in superstring theory. Essentially, Calabi–Yau manifolds are shapes that satisfy the requirement of space for the six "unseen" spatial dimensions of string theory, which may be smaller than our currently observable lengths as they have not yet been detected.
  https://observablehq.com/@sw1227/calabi-yau-manifold-3d
  
#### Mountain Terrain 3D
  3D Visualization of Mt. Nabewari in Japan.
  https://observablehq.com/@sw1227/mountain-terrain-3d
  https://observablehq.com/@sw1227/contoured-mountainious-terrai-3d
  
#### Sea Level 3D
  https://observablehq.com/@andreasplesch/sea-level-past-present-and-future
  
#### Asteroid Itokawa 3D
  https://observablehq.com/@sw1227/asteroid-itokawa-3d-9mb
  
#### Interactive 3D L-system renderings
  https://observablehq.com/@kelleyvanevert/3d-l-systems
  
#### 3D UMAP-js + ScatterGL
  https://observablehq.com/@radames/3d-umap-js-scattergl
  https://observablehq.com/@radames/scattergl-3d-sphere-points
  
#### Simple 3D Redshift Explorations
  https://observablehq.com/@miketahani/simple-3d-redshift-explorations
  
#### 3D Phyllotaxis
  https://observablehq.com/@vasturiano/3d-phyllotaxis
  
#### 3D Clustered Bubbles 2
  https://observablehq.com/@ryshackleton/3d-clustered-bubbles-2
  
#### clx. Hello, 3D geographic contours!
  Here’s a quick demo of 3D contours using three-geo.
  https://observablehq.com/@wolfiex/clx-hello-3d-geographic-contours
  
#### Drawing indexed mesh data as screen-space normals without duplicating data
  https://observablehq.com/@rreusser/debugging-normals-without-duplicating-geometry-data
  
#### GPGPU Simplex noise
  https://observablehq.com/@tezzutezzu/simplex-noise-on-the-gpu


#### Exploring CSG using THREE-CSGMesh
  https://observablehq.com/@k9/exploring-csg-using-three-csgmesh
  https://github.com/manthrax/THREE-CSGMesh
  
#### Flying Cubes
  http://examples.claygl.xyz/examples/basicFlyingCubes.html
  http://examples.claygl.xyz/
  
#### 3dg-viz
  A WebGL distributed graph visualization library
  https://demo-3dg-viz.herokuapp.com/
  http://nodexlgraphgallery.org/Pages/AboutNodeXL.aspx
  
#### ThreeGraph
  ThreeGraph is an interactive web application for exploring connected networks by means of a 3D graph. An example graph is at http://rockstars.jansson.me.uk. Scroll to the bottom of this page for more info about this graph.
   https://github.com/andreasjansson/ThreeGraph
  
#### jagenjo/rendeer.js
  Light-weight 3D Scene graph library with renderer in WebGL
  https://github.com/jagenjo/rendeer.js
  http://tamats.com/projects/rendeer/examples


  
